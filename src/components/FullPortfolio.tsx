import { useState } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { FileText, ExternalLink, Briefcase, Image, X } from "lucide-react";

// Note: Adjust the "../" path if your component is located deeper in your folder structure
import recommendationLetter from "../assets/Recommendation_letter_fila.jpg";

const FullPortfolio = () => {
  // State to control if the image modal is open or closed
  const [isModalOpen, setIsModalOpen] = useState(false);

  return (
    <>
      <section id="portfolio" className="py-24 bg-secondary">
        <div className="container mx-auto px-6">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="max-w-4xl mx-auto"
          >
            <div className="text-center mb-12">
              <h2 className="text-4xl md:text-5xl font-heading font-bold text-primary mb-4">
                Portfolio & CV
              </h2>
              <p className="text-lg text-muted-foreground">
                Explore the complete collection of my architectural work and professional background
              </p>
            </div>

            <div className="space-y-8">
              {/* Portfolio Card */}
              <div className="bg-card rounded-lg shadow-soft overflow-hidden">
                <div className="grid md:grid-cols-2 gap-0">
                  {/* Preview/Thumbnail side */}
                  <div className="bg-gradient-hero p-12 flex items-center justify-center">
                    <div className="text-center">
                      <FileText className="w-24 h-24 text-primary mx-auto mb-6" />
                      <p className="text-sm uppercase tracking-wider text-muted-foreground mb-2">
                        Complete Portfolio
                      </p>
                    </div>
                  </div>

                  {/* Content side */}
                  <div className="p-12 flex flex-col justify-center">
                    <h3 className="text-2xl font-heading font-bold text-primary mb-4">
                      Architecture & Design Portfolio
                    </h3>
                    <p className="text-foreground mb-6 leading-relaxed">
                      This comprehensive portfolio showcases my journey through architectural design, 
                      urban planning, and interior design projects. It includes detailed case studies, 
                      technical drawings, 3D visualizations, and conceptual explorations.
                    </p>
                    
                    <div className="space-y-3 mb-8">
                      <div className="flex items-center gap-2 text-sm text-muted-foreground">
                        <div className="w-1.5 h-1.5 bg-blue-accent rounded-full" />
                        Residential & Commercial Projects
                      </div>
                      <div className="flex items-center gap-2 text-sm text-muted-foreground">
                        <div className="w-1.5 h-1.5 bg-blue-accent rounded-full" />
                        Urban Planning Concepts
                      </div>
                      <div className="flex items-center gap-2 text-sm text-muted-foreground">
                        <div className="w-1.5 h-1.5 bg-blue-accent rounded-full" />
                        Interior Design Studies
                      </div>
                      <div className="flex items-center gap-2 text-sm text-muted-foreground">
                        <div className="w-1.5 h-1.5 bg-blue-accent rounded-full" />
                        Technical Documentation
                      </div>
                    </div>

                    <a
                      href="https://drive.google.com/file/d/1iFOGHYR8Cmq2LPtVHj5EFUzLawrcPvHI/view?usp=sharing"
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center justify-center gap-2 px-8 py-4 bg-primary text-primary-foreground rounded-lg hover:bg-blue-accent transition-all shadow-soft hover:shadow-hover group w-fit"
                    >
                      View Full Portfolio
                      <ExternalLink className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
                    </a>
                  </div>
                </div>
              </div>

              {/* CV / Resume Card */}
              <div className="bg-card rounded-lg shadow-soft overflow-hidden">
                <div className="grid md:grid-cols-2 gap-0">
                  {/* Preview/Thumbnail side (Reversed on desktop for visual balance) */}
                  <div className="bg-gradient-hero p-12 flex items-center justify-center md:order-2">
                    <div className="text-center">
                      <Briefcase className="w-24 h-24 text-primary mx-auto mb-6" />
                      <p className="text-sm uppercase tracking-wider text-muted-foreground mb-2">
                        Curriculum Vitae
                      </p>
                    </div>
                  </div>

                  {/* Content side */}
                  <div className="p-12 flex flex-col justify-center md:order-1">
                    <h3 className="text-2xl font-heading font-bold text-primary mb-4">
                      Professional Experience
                    </h3>
                    <p className="text-foreground mb-6 leading-relaxed">
                      Review my professional background, educational history, software proficiencies, 
                      and complete work experience in detail to see how I can bring value to your next project.
                    </p>
                    
                    <div className="space-y-3 mb-8">
                      <div className="flex items-center gap-2 text-sm text-muted-foreground">
                        <div className="w-1.5 h-1.5 bg-blue-accent rounded-full" />
                        Work Experience & Roles
                      </div>
                      <div className="flex items-center gap-2 text-sm text-muted-foreground">
                        <div className="w-1.5 h-1.5 bg-blue-accent rounded-full" />
                        Education & Degrees
                      </div>
                      <div className="flex items-center gap-2 text-sm text-muted-foreground">
                        <div className="w-1.5 h-1.5 bg-blue-accent rounded-full" />
                        Software & Technical Skills
                      </div>
                      <div className="flex items-center gap-2 text-sm text-muted-foreground">
                        <div className="w-1.5 h-1.5 bg-blue-accent rounded-full" />
                        Certifications & Awards
                      </div>
                      <div className="flex items-center gap-2 text-sm text-muted-foreground">
                        <div className="w-1.5 h-1.5 bg-blue-accent rounded-full" />
                        Letters of Recommendation
                      </div>
                    </div>

                    <div className="flex flex-wrap items-center gap-4">
                      <a
                        href="https://drive.google.com/file/d/1Nh7Y88gsVZ-QIJLM_8C2r_88WLj8b7k4/view?usp=sharing"
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center justify-center gap-2 px-8 py-4 bg-primary text-primary-foreground rounded-lg hover:bg-blue-accent transition-all shadow-soft hover:shadow-hover group w-fit"
                      >
                        View CV
                        <ExternalLink className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
                      </a>
                      
                      {/* Changed from <a> to <button> to open the modal instead of a new tab */}
                      <button
                        onClick={() => setIsModalOpen(true)}
                        className="inline-flex items-center justify-center gap-2 px-8 py-4 border-2 border-primary text-primary rounded-lg hover:bg-primary/5 transition-all group w-fit cursor-pointer"
                      >
                        View Recommendation
                        <Image className="w-5 h-5 group-hover:scale-110 transition-transform" />
                      </button>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </motion.div>
        </div>
      </section>

      {/* --- Image Modal Overlay --- */}
      <AnimatePresence>
        {isModalOpen && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            // Clicking the dark background closes the modal
            onClick={() => setIsModalOpen(false)} 
            className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 sm:p-8 backdrop-blur-sm cursor-pointer"
          >
            <motion.div
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
              // Prevent clicks on the image itself from closing the modal
              onClick={(e) => e.stopPropagation()} 
              className="relative max-w-4xl w-full max-h-full flex flex-col items-center justify-center cursor-default"
            >
              {/* Close Button */}
              <button
                onClick={() => setIsModalOpen(false)}
                className="absolute -top-12 right-0 text-white hover:text-gray-300 transition-colors p-2 cursor-pointer"
                aria-label="Close"
              >
                <X className="w-8 h-8" />
              </button>

              {/* The Image */}
              <img
                src={recommendationLetter}
                alt="Letter of Recommendation"
                className="w-auto h-auto max-w-full max-h-[85vh] object-contain rounded-lg shadow-2xl"
              />
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
    </>
  );
};

export default FullPortfolio;
